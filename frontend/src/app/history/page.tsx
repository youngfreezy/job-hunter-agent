...
export default function HistoryPage() {
    ...
    try {
      ...
      const results: SessionWithApps[] = await Promise.all(
        allSessions.map(async (session) => {
          ...
          const { timeSaved, automationTime, manualEstimate } =
            computeTimeSaved(entries);
  ...
  const totals = useMemo(() => {
    const totalTimeSaved = sessionsWithApps.reduce(
      (sum, s) => sum + s.timeSaved,
      0
    );
    const totalApps = sessionsWithApps.reduce(
      ...
      <nav className="sticky top-0 z-50 border-b border-border/70 bg-background/95 px-6 py-3 shadow-sm supports-[backdrop-filter]:bg-background/90 supports-[backdrop-filter]:backdrop-blur-md">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/history"
              className="px-3 py-1.5 text-sm font-medium rounded-md bg-muted text-foreground transition-colors"
            >
              History
            </Link>
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-1">
            See how much time you&apos;ve saved and track every session from
            start to finish.
          </p>
        </div>
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              <p className="text-lg font-medium text-muted-foreground">
                No sessions yet. Start your first search and we&apos;ll track
                your progress here.
              </p>
            </CardContent>
          </Card>
                          ...
                          <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <Badge
                                  variant={
                                    STATUS_COLORS[session.status] || "outline"
                                  }
                                >
                        ...
                        {isExpanded && (
                          <div className="px-5 pb-5 border-t border-border/50">
                            ...
                            <div className="flex gap-2 mt-4">
                              ...
                              <Link
                                href={`/session/${session.session_id}/manual-apply`}
                              >
                                <Button size="sm" variant="outline">
                                  Review Applications
                                </Button>
