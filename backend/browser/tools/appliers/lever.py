...
"""Lever ATS applier — single-page application form."""
...
class LeverApplier(BaseApplier):
    """Apply to jobs on jobs.lever.co via Playwright form filling."""
    ...
    async def apply(
        ...
        try:
            # Step 1: Click Apply button (if on posting page)
            await self._emit_step("Looking for Apply button...")
            has_form = await self.page.query_selector('form[action*="apply"], .application-form')
            if not has_form:
                ...
                if not clicked:
                    ...
                    if not has_form:
                        return self._make_result(
                            ...
                            error_message="Could not find Apply button or form",
                        )
                await self._random_delay(1.0, 2.0)
                await self._wait_for_navigation()

            # Step 2: Fill the single-page form
            ...
            # Step 4: Post-submit check
            await self._capture_screenshot(job)
            return await self._post_submit_check(job_id, cover_letter)

        except Exception as exc:
            logger.error("Lever apply failed for %s: %s", job.title, exc, exc_info=True)
            return self._make_result(
                job_id, ApplicationStatus.FAILED,
                error_message=f"Lever apply error: {str(exc)[:200]}",
            )
