...
test.describe("Manual Apply (Live Integration)", () => {
  test("manual-apply UI shows persisted submitted entry with cover letter and tailored resume", async ({
    ...
    expect((submitted?.tailored_resume?.tailored_text ?? "").length).toBeGreaterThan(50);
    ...
    await expect(page.getByText("Applied AI Engineer, Beneficial Deployments")).toBeVisible({
      timeout: 20_000,
    });
    ...
    const resumeDownload = page
      .getByRole("button", {
        name: "Download PDF",
      })
      .nth(1);
    const [resumePdf] = await Promise.all([page.waitForEvent("download"), resumeDownload.click()]);
